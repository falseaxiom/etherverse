...
class Terrain extends Group {
  constructor(parent) {
    ...
    this.geometry = new PlaneGeometry(
      terrainSize.width,
      terrainSize.height,
      terrainSize.vertsWidth - 1,
      terrainSize.vertsHeight - 1
    );
    ...
    folder
      .add(this.state, "exaggeration", 0, 70)
      .onChange(() => this.updateTerrainGeo());
    folder
      .add(this.state, "waterLevel", -100, 100)
      .name("Water Level")
      .onChange(() => this.updateTerrainGeo());
    folder
      ...
      .onChange(() => this.updateTerrainGeo());
    folder
      .add(this.state, "middleGradient", 0, 1)
      .name("Peak Height")
      .onChange(() => this.updateTerrainGeo());
    folder
    ...
    this.geometry.faces.forEach((f) => {
      ...
      if (max <= this.state.waterLevel) {
        return f.color.setRGB(
          (this.state.waterColor.r + Math.random() * wiggle) / 255,
      ...
      if (max - this.state.waterLevel > this.state.exaggeration * 7)
        return f.color.setRGB(
          (this.state.peakColor.r + Math.random() * wiggle) / 255,
          (this.state.peakColor.g + Math.random() * wiggle) / 255,
          (this.state.peakColor.b + Math.random() * wiggle) / 255
        );
      ...
      if (ratio >= this.state.middleGradient) {
        ...
        return f.color.setRGB(
          (this.state.peakColor.r * ratio +
            this.state.middleColor.r * (1 - ratio) +
            Math.random() * wiggle) /
            255,
          (this.state.peakColor.g * ratio +
            this.state.middleColor.g * (1 - ratio) +
            Math.random() * wiggle) /
            255,
          (this.state.peakColor.b * ratio +
            this.state.middleColor.b * (1 - ratio) +
            Math.random() * wiggle) /
            255
        );
      }
      ...
      return f.color.setRGB(
        (this.state.middleColor.r * ratio +
          this.state.bankColor.r * (1 - ratio) +
          Math.random() * wiggle) /
          255,
        (this.state.middleColor.g * ratio +
          this.state.bankColor.g * (1 - ratio) +
          Math.random() * wiggle) /
          255,
        (this.state.middleColor.b * ratio +
          this.state.bankColor.b * (1 - ratio) +
          Math.random() * wiggle) /
          255
      );
    });
  ...
  generateTexture() {
    ...
    for (let i = 0; i < terrainSize.vertsHeight; i++) {
      for (let j = 0; j < terrainSize.vertsWidth; j++) {
        let v = this.octave(
          i / terrainSize.vertsWidth,
          j / terrainSize.vertsHeight,
          this.state.octaves,
          simplex
        );
        canvas[i][j] = v;
