...
class ChunkManager extends Group {
  constructor(parent) {
    ...
    for (let i = 0; i < coordinates.length; i++) {
      let new_plane_geo = new PlaneGeometry(
        this.state.chunkWidth,
        this.state.chunkWidth,
        this.state.chunkVertWidth - 1,
        this.state.chunkVertWidth - 1
      );
    ...
    // add water plane
    var waterGeometry = new PlaneBufferGeometry(
      this.state.chunkWidth * 3,
      this.state.chunkWidth * 3
    );
    ...
    folder0
      .add(this.state, "octaves", 1, 16)
      .name("Jaggedness")
      .onChange(() => this.updateNoise());
    folder0
      .add(this.state, "freq", 1, 10)
      .name("Peaks")
      .onChange(() => this.updateNoise());
    folder0
      .add(this.state, "randSeed", 0, 10)
      .name("World Seed")
      .onChange(() => this.updateSimplexSeed());
    ...
    folder
      .add(this.state, "ogExaggeration", 0, 70)
      .name("Exaggeration")
      .onChange(() => this.updateExaggeration());
    folder
      .add(this.state, "power", 0, 5)
      .name("Valleys")
      .onChange(() => this.updateTerrainGeo());
    folder
      .add(this.state, "waterLevel", -200, 200)
      .name("Water Level")
      .onChange(() => this.updateWaterLevel());
    folder
      .add(this.state, "colorWiggle", -1, 1)
      .name("Color Texturing")
      .onChange(() => this.updateTerrainGeo());
    folder
      .add(this.state, "middleGradient", 0.1, 1)
      .name("Gradient")
      .onChange(() => this.updateTerrainGeo());
    folder
      .addColor(this.state, "waterColor")
      .name("Ocean Color")
      .onChange(() => this.updateTerrainGeo());
    folder
      .addColor(this.state, "bankColor")
      .name("Bank Color")
      .onChange(() => this.updateTerrainGeo());
    folder
      .addColor(this.state, "middleColor")
      .name("Middle Color")
      .onChange(() => this.updateTerrainGeo());
    folder
      .addColor(this.state, "peakColor")
      .name("Peak Color")
      .onChange(() => this.updateTerrainGeo());
    folder.add(this.state, "terraced").onChange(() => this.updateTerrainGeo());
    folder
      .add(this.state, "terraces", 1, 20)
      .name("Num Terraces")
      .onChange(() => this.updateTerrainGeo());
    this.state.gui
      .add(this.state, "preset", PRESET_NAMES)
      .name("Terrain Presets")
      .onChange(() => this.loadPreset());
    this.state.gui
      .add(this.state, "updateWithMusic")
  ...
  update(timeStamp, x, y, z) {
    ...
    if (z > this.state.chunkWidth / 2) {
      ...
      this.state.chunks[0] = new Chunk(
        this,
        this.state.chunkWidth + this.state.currentXOffset,
        0,
        this.state.chunkWidth + this.state.currentZOffset,
        plane_geos[0]
      );
      this.state.chunks[1] = new Chunk(
        this,
        this.state.currentXOffset,
        0,
        this.state.chunkWidth + this.state.currentZOffset,
        plane_geos[1]
      );
      ...
      this.state.chunks[3] = new Chunk(
        this,
        this.state.chunkWidth + this.state.currentXOffset,
        0,
        this.state.currentZOffset,
        plane_geos[1]
      );
      this.state.chunks[6] = new Chunk(
        this,
    ...
    if (need_update) {
      ...
      // top row
      this.state.chunks[0].setChunkPosition(
        this.state.chunkWidth,
        0,
        this.state.chunkWidth
      );
      this.state.chunks[1].setChunkPosition(0, 0, this.state.chunkWidth);
      this.state.chunks[2].setChunkPosition(
        -this.state.chunkWidth,
        0,
        this.state.chunkWidth
      );
      ...
      // bottom row
      this.state.chunks[6].setChunkPosition(
        this.state.chunkWidth,
        0,
        -this.state.chunkWidth
      );
      this.state.chunks[7].setChunkPosition(0, 0, -this.state.chunkWidth);
      this.state.chunks[8].setChunkPosition(
        -this.state.chunkWidth,
        0,
        -this.state.chunkWidth
      );
