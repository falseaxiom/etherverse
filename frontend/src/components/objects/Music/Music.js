...
class Music extends Group {
  constructor(parent, camera) {
    ...
    folder
      ...
      .onChange((audiofile) => this.updateAudioFile(audiofile));
    folder.open();
  ...
  update() {
    if (
      this.state.soundUpdate % 5 == 0 &&
      this.parent.chunkmanager.state.updateWithMusic == true
    ) {
      ...
      if (avg > 10) {
        chunkManager.state.exaggeration =
          (chunkManager.state.ogExaggeration * avg) / 50;
        let factor = avg / 500;

        chunkManager.state.bankColor = new Color(
          chunkManager.state.bankColor.r,
          chunkManager.state.bankColor.g,
          chunkManager.state.bankColor.b
        );
        chunkManager.state.middleColor = new Color(
          chunkManager.state.middleColor.r,
          chunkManager.state.middleColor.g,
          chunkManager.state.middleColor.b
        );
        chunkManager.state.peakColor = new Color(
          chunkManager.state.peakColor.r,
          chunkManager.state.peakColor.g,
          chunkManager.state.peakColor.b
        );

        if (this.state.colorLevel == 0) {
          chunkManager.state.bankColor.lerp(
            chunkManager.state.middleColor,
            factor
          );
        } else if (this.state.colorLevel == 1) {
          chunkManager.state.middleColor.lerp(
            chunkManager.state.peakColor,
            factor
          );
        } else {
          chunkManager.state.peakColor.lerp(
            chunkManager.state.bankColor,
            factor
          );
        }
